...
switch ($op) {
		...
		if ($numrows > 0) {
			...
			foreach (array_keys($instr_arr) as $i) {
				...
				$GLOBALS['xoopsTpl']->append('insListInstr', [
                                                      'instrid' => $insinstr_instrid,
                                                      'title' => $insinstr_title,
                                                      'status' => $insinstr_status,
                                                      'pages' => $insinstr_pages,
                                                      'ctitle' => $insinstr_cat->getVar( 'title' ),
                                                      'cid' => $insinstr_cat->getVar( 'cid' ),
                                                      'class' => $class
                                                      ]);
			}
	...
	case 'saveinstr':
		...
		if ( ! $cid ) {
			$err = true;
			$message_err .= _AM_INSTRUCTION_ERR_CAT
                      . '<br>';
		}
		// Проверка названия
		if ( !$instr_title ) {
			$err = true;
			$message_err .= _AM_INSTR_ERR_TITLE
                      . '<br>';
		}
		// Проверка основного текста
		if ( !$instr_description ) {
			$err = true;
			$message_err .= _AM_INSTR_ERR_DESCRIPTION
                      . '<br>';
		}
      ...
      $adminObject->displayNavigation(basename(__FILE__));
			...
			$message_err = '<div class="errorMsg" style="text-align: left;">'
                      . $message_err
                      . '</div>';
	...
	case 'delinstr':
      ...
      $adminObject->displayNavigation(basename(__FILE__));
			...
			xoops_confirm([
                    ...
                    'instrid' => $instrid,
                    'op' => 'delinstr'
                    ], 'instr.php', sprintf(_AM_INSTRUCTION_FORMDELINSTR, $objInsinstr->getVar('title')));
			// Текст внизу админки
			include __DIR__ . '/admin_footer.php';
	...
	case 'savepage':
		...
		if (! $pageid && ! $instrid) {
			$err = true;
			$message_err .= _AM_INSTRUCTION_ERR_INSTR
                      . '<br>';
		}
		// Проверка веса
		if ($weight == 0){
			$err = true;
			$message_err .= _AM_INSTRUCTION_ERR_WEIGHT
                      . '<br>';
		}
		// Проверка родительской страницы
		if ($pageid && ($pageid == $pid)) {
			$err = true;
			$message_err .= _AM_INSTRUCTION_ERR_PPAGE
                      . '<br>';
		}
		// Проверка названия
		if (!$page_title) {
		...
		if ( !$page_hometext ) {
			$err = true;
			$message_err .= _AM_INSTR_ERR_HOMETEXT
                      . '<br>';
		}
      ...
      $adminObject->displayNavigation(basename(__FILE__));
		...
		// Если небыло ошибок
			...
			if ($inspage_Handler->insert($objInspage)) {
				...
				$redirect_url = 'instr.php?op=viewinstr&amp;instrid='
                        . $instrid
                        . '#pageid_'
                        . $pid;
	...
	case 'delpage':
      ...
      $adminObject->displayNavigation(basename(__FILE__));
			...
			xoops_confirm([
                    ...
                    'pageid' => $pageid,
                    'op' => 'delpage'
                    ], 'instr.php', sprintf( _AM_INSTRUCTION_FORMDELPAGE, $objInspage->getVar('title')));
			// Текст внизу админки
			include __DIR__ . '/admin_footer.php';
