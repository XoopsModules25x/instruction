...
include_once $moduleDirName . '/include/functions.php';
  ...
  //$xoTheme->addStylesheet( XOOPS_URL . '/modules/instruction/css/blocks.css' );
	...
	if( is_array($cat_view) && count($cat_view) > 0) {
		...
		$sql = "SELECT p.pageid, p.instrid, p.title, p.dateupdated, i.title, i.cid FROM {$inspage_Handler->table} p, {$insinstr_Handler->table} i WHERE p.instrid = i.instrid AND i.cid IN ("
            . implode(', ', $cat_view)
            . ") AND p.status > 0 AND i.status > 0 ORDER BY p.dateupdated DESC";
		// Лимит запроса
...
function b_instr_lastpage_edit($options = []) {
	...
	$form = '';
	$form .= _MB_INSTR_DISPLAYPAGESC
          . ' <input name="options[0]" size="5" maxlength="255" value="'
          . $options[0]
          . '" type="text" /><br>'
          . "\n";
	$form .= _MB_INSTR_NUMCHARSC
