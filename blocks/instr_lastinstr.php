...
function b_instr_lastinstr_show($options = []) {
  ...
  //$xoTheme->addStylesheet( XOOPS_URL . '/modules/instruction/css/blocks.css' );
	...
	if(is_array($cat_view) && count($cat_view) > 0) {
		...
		$sql = "SELECT `instrid`, `cid`, `title`, `pages`, `dateupdated` FROM {$insinstr_Handler->table} WHERE `cid` IN ("
            . implode( ', ', $cat_view )
            . ") AND `status` > 0 ORDER BY `dateupdated` DESC";
		// Лимит запроса
