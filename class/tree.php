...
class InstructionTree extends XoopsObjectTree {
	...
	public function _makePagesAdminOptions($key, &$ret, $prefix_orig, $objInsinstr, $class = 'odd', $prefix_curr = '') {
		...
		if ($key > 0) {
			...
			// Действие - удаление
			$act_del = ($num_childs > 0) ? '<img src="../assets/icons/no_delete_mini.png" alt="'
                                        . _AM_INSTR_NODELPAGE
                                        . '" title="'
                                        . _AM_INSTR_NODELPAGE
                                        . '" />' : '<a href="instr.php?op=delpage&pageid='
                                        . $pageid
			...
			$ret .= '<tr class="'
              ...
              . ' '
              . $page_link
              . '</td><td style="text-align:center; width:50px;"><input type="text" name="weights[]" size="2" value="'
              . $pageweight
              . '" /><input type="hidden" name="pageids[]" value="'
              . $pageid
              . '" /></td><td style="text-align:center; width:180px;">';
			// Просмотре без кэша
			$ret .= ' <a href="'
              . XOOPS_URL
              . '/modules/'
              ...
              . '/page.php?id='
              . $pageid
              . '&amp;nocache=1"><img src="../assets/icons/no_cache.png" alt="'
              . _AM_INSTR_DISPLAY_NOCACHE
              . '" title="'
              . _AM_INSTR_DISPLAY_NOCACHE
              . '" /></a> ';
			// Добавить подстраницу
			$ret .= ' <a href="instr.php?op=editpage&instrid='
              . $instrid
              . '&pid='
              . $pageid
              . '"><img src="../assets/icons/add_mini.png" alt="'
              . _AM_INSTRUCTION_ADDSUBPAGE
              . '" title="'
              . _AM_INSTRUCTION_ADDSUBPAGE
              . '" /></a> ';

			if( $pagestatus ){
				$ret .= ' <img src="../assets/icons/lock_mini.png" alt="'
                . _AM_INSTRUCTION_LOCK
                . '" title="'
                . _AM_INSTRUCTION_LOCK
                . '"> ';
			} else {
				$ret .= ' <img src="../assets/icons/unlock_mini.png" alt="'
                . _AM_INSTRUCTION_UNLOCK
                . '" title="'
                . _AM_INSTRUCTION_UNLOCK
                . '"> ';
			}

			$ret .= ' <a href="instr.php?op=editpage&pageid='
              . $pageid
              . '"><img src="../assets/icons/edit_mini.png" alt="'
              . _AM_INSTRUCTION_EDIT
              . '" title="'
              . _AM_INSTRUCTION_EDIT
              . '"></a> '
              . $act_del
	...
	public function makePagesAdmin(&$objInsinstr, $prefix = '-', $key = 0) {
		$ret = '<form name="inspages" action="instr.php" method="post"><table class="outer" style="width:100%; border-spacing:1;"><tr>
            <th align="center" colspan="3">'
            . sprintf(_AM_INSTRUCTION_LISTPAGESININSTR, $objInsinstr->getVar('title'))
            . '</th></tr><tr><td class="head" style="text-align:center;">'
            . _AM_INSTRUCTION_TITLE
            . '</td><td class="head" style="text-align:center; width:50px;">'
            . _AM_INSTRUCTION_WEIGHT
            . '</td><td class="head" style="text-align:center; width:180px;">'
            . _AM_INSTRUCTION_ACTION
            . '</td></tr>';

		// Выводим все страницы
		$this->_makePagesAdminOptions($key, $ret, $prefix, $objInsinstr);

		$ret .= '<tr class="foot"><td><a href="instr.php?op=editpage&instrid='
            . $objInsinstr->getVar('instrid')
            . '"><img src="../assets/icons/add_mini.png" alt="'
            . _AM_INSTRUCTION_ADDPAGE
            . '" title="'
            . _AM_INSTRUCTION_ADDPAGE
            . '"></a></td><td colspan="2"><input type="hidden" name="instrid" value="'
            . $objInsinstr->getVar('instrid')
            . '" /><input type="hidden" name="op" value="updpage" /><input type="submit" value="'
            . _SUBMIT
            . '" /></td></tr></table></form>';
	...
	public function _makeCatsAdminOptions($key, &$ret, $prefix_orig, $cidinstrids = [], $class = 'odd', $prefix_curr = '') {
		if ( $key > 0 ) {
			...
			// Действие - удаление
			$act_del = (($num_instrs > 0) || ($num_childs > 0)) ? '<img src="../assets/icons/no_delete_mini.png" alt="'
                                                            . _AM_INSTR_NODELCAT
                                                            . '" title="'
                                                            . _AM_INSTR_NODELCAT
                                                            . '" />' : '<a href="cat.php?op=delcat&cid='
                                                            . $catid
                                                            . '"><img src="../assets/icons/delete_mini.png" alt="'
                                                            . _AM_INSTRUCTION_DEL
                                                            . '" title="'
                                                            ...
                                                            . '" /></a>';
			// Действие - просмотр
			$act_view = ($num_instrs > 0) ? '<a href="instr.php?cid='
                                      . $catid
                                      . '"><img src="../assets/icons/view_mini.png" alt="'
                                      . _AM_INSTR_VIEWINSTR
                                      . '" title="'
                                      . _AM_INSTR_VIEWINSTR
                                      . '" /></a>' : '<img src="../assets/icons/no_view_mini.png" alt="'
	...
	public function makeCatsAdmin($prefix = '-', $cidinstrids = [], $key = 0) {
		$ret = '<table class="outer" style="width:100%; border-spacing:1;"><tr><th style="text-align:center;" colspan="4">'
            . _AM_INSTR_LISTALLCATS
            . '</th></tr><tr><td class="head">'
            . _AM_INSTRUCTION_TITLE
            . '</td><td class="head" style="text-align:center; width:50px;">'
            . _AM_INSTRUCTION_WEIGHT
            . '</td><td class="head" style="text-align:center; width:100px;">'
            . _AM_INSTR_INSTRS
            . '</td><td class="head" style="text-align:center; width:150px;">'
            . _AM_INSTRUCTION_ACTION
            . '</td></tr>';
	...
	public function _makePagesUserTree($key, &$ret, $currpageid = 0, &$lastpageids = [], $level = 0) {
		...
		if ($key > 0) {
			...
			if($pagetype == 0) {
				$ret .= '<span class="InstrTreeEmptyPage">'
                . $pagetitle
                . '</span>';
			//
			} elseif( $currpageid == $pageid ) {
	...
	public function makePagesUser($currpageid = 0, &$prevpages = [], &$nextpages = [], $key = 0) {
		...
		$ret = '<div onclick="instr_tree_toggle(arguments[0])"><div>'
            . _MD_INSTRUCTION_LISTPAGES
            . '</div><div><ul class="InstrTreeContainer">';
